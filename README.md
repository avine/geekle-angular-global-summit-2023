...
La solution qu'il propose pour Angular utilise le paquet [module-federation-plugin](https://github.com/angular-architects/module-federation-plugin) et exploite le "Module Federation" introduit dans [Webpack 5](https://webpack.js.org/).
...
## Un peu d'échafaudage

Commençons par créer les 3 grandes parties de notre monorepo :
...
Notez qu'au départ la donnéee est `null`.
...
Nous allons utiliser la shematic [@angular-architects/module-federation](https://www.npmjs.com/package/@angular-architects/module-federation) pour ajouter des supers pouvoirs à nos applications :
...
Celui-ci permet de surcharger la configuration Webpack de n'importe quel projet du workspace, puisque celle-ci nous sont masquée par la CLI d'Angular.
...
Mais avant d'expliquer le contenu de ces fichiers, voyons ce que contient l'application "remote1" compilée.
...
Celui-ci inique sans ambiguité où aller récupérer le `remoteEntry.js` du "remote1" :
...
Désormais l'application est chargée dynamiquement après que le manifest et ses différents `remoteEntry.js` ont bien été téléchargées :
...
## Le routage Angular vers le Micro Frontend

Nous sommes presque ! Il ne nous reste plus qu'à naviguer vers le Micro Frontend depuis le "shell" grâce à un simple Routing Angular :
...
Souvenez-vous, le "remote1" affiche la donnée `data$` du service `SharedService`.
Et cette donnée a bien été initialisée par la fonction `setData` appelée depuis le "shell".
Et c'est pouquoi, le texte `"Hello Micro Frontend"` s'affiche bien dans le Micro Frontend.
